...
public class Controller implements Observer{
	...
	/**Constructor that takes in a virtual vending machine and the labels for each button
	...
	public Controller(VendingMachine vendingMachine, String[] buttonLabel) {
		...
		sbListener = new SBListener[buttonLabel.length];
		for(int x = 0; x < buttonLabel.length; x++) {
			sbListener[x] = new SBListener(x, buttonLabel[x]);
	...
	/** Setup the a new instance of the vending machine with values passed in from calling class
	 * @param validCoins
	 * @param buttonLabel
	 ...
	 */
	public Controller(int[] validCoins, String[] buttonLabel, int coinRackCapacity, String[] popCanNames, int receptacleCapacity) {


		//TODO: throw exception if button labels and popcan names are not equal
	...
	public void update(Observable listener, Object obj)
	   {
			...
			if (listener == csListener)
	      {
	    	  ...
	    	  //System.out.println("CSListener event reporting!"); //Just to test if it is functioning
	      }
	      ...
	      if (listener == activatedButton)
	      {
	    	  ...
	    	  case "Pressed":
	    		  int buttonID = activatedButton.getID();
	    		  if (balance == 200) {
	    			  vendingMachine.getPopCanRack(activatedButton.getID());
	    		  }
	    		  //TODO:if enough money dispense pop, otherwise...
